...
const handler = async (event) => {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Missing NETLIFY_EMAILS_PROVIDER_API_KEY environment variable');
      return {
        statusCode: 500,
    ...
    if (!sendgridResponse.ok) {
      ...
      console.error('Sendgrid API error:', sendgridResponse.status, errorData);
      
      // For 4xx errors (client errors like invalid email), return 400
      if (sendgridResponse.status >= 400 && sendgridResponse.status < 500) {
        return {
          statusCode: 400,
          body: JSON.stringify({ error: 'Invalid subscription request' }),
        };
      }
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: 'Successfully subscribed to our mailing list!' }),
    };
  } catch (error) {
